...
## Create the model.
mlp = vg.MLP_M(input_size=num_features, output_size=num_classes)
...
for epoch in range(args.epochs):
    ...
    if epoch % 1 == 0:
        ...
        print(f'      Valid accuracy: {acc:.4}')

        t_o = mlp(vg.TensorNode(xtrain))
        otrain = t_o.value

        predictions = np.argmax(otrain, axis=1)
        num_correct = (predictions == ytrain).sum()
        acc = num_correct / ytrain.shape[0]
        relu_train.append(acc)

        o.clear()  # gc the computation graph

        print(f'      train accuracy: {acc:.4}')
...
wei_val = np.array(relu_val)
wei_train = np.array(relu_train)
# np.save('./results/wei_val', wei_val)
# np.save('./results/wei_train', wei_train)